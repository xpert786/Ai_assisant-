...
class MessageCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        if message.content:
            ...
            # Detect and set source based on content
            source = ai_client.detect_source_from_content(message.content)
            if source:
                message.source = source
                logger.info(f"Message source detected: {source}")

            # Extract client information and update fields
            client_name, client_contact = ai_client.extract_client_info(message.content)
            logger.info("Client information extracted from message")
            if client_name:
                message.client_name = client_name
                logger.info(f"Client name set: {client_name}")
            if client_contact:
            ...
            # Extract text from image (optimize first to avoid 502/timeouts on large uploads)
                ...
                if extracted_text and len(extracted_text.strip()) > 10:
                    ...
                    # Detect and set source based on content
                    source = ai_client.detect_source_from_content(extracted_text)
                    if source:
                        message.source = source
                    ...
                    # Extract full message content, not just the first line
                    full_content = extracted_text.strip()
                    message.content = full_content
                    
                    # Enhanced name extraction from content
                    if not client_name:
                        # Try to find names in different formats
                        name_patterns = [
                            # Group title with company and person
                            r'([A-Z][A-Za-z0-9\s&\-\.]+(?:LLC|Ltd|Corporation|Corp|Inc|Limited|Trading|Solutions|Group))(?:\s*[-:]\s*([A-Z][A-Za-z\s&]+))',
                            # Message sender name
                            r'(?<=\n)([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)+)(?=:)',
                            # Mentioned names
                            r'@([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)+)',
                            # Number of members with names
                            r'(\d+)\s*members?\s*(?:[-:]\s*)?([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)+)'
                        ]
                        
                        for pattern in name_patterns:
                            name_match = re.search(pattern, full_content, re.MULTILINE)
                            if name_match:
                                # If we have a second group (person name after company/members)
                                if name_match.group(2):
                                    client_name = name_match.group(2)
                                else:
                                    client_name = name_match.group(1)
                                logger.info(f"Extracted client name using pattern: {pattern}")
                                logger.info(f"Found name: {client_name}")
                                break
                                
                        # Log if no name was found
                        if not client_name:
                            logger.warning("No client name could be extracted from the content")
        ...
        # Set the message ID in the session so we can display it
        self.request.session['new_message_id'] = message.id
        logger.debug(f"Message ID {message.id} set in session")
        ...
        if 'previous_messages' in self.request.session:
            ...
            logger.debug("Cleared previous messages from session")
...
def ajax_enhance_response(request, pk):
    ...
    # Call OpenAI using AIClient
    try:
        # Create a temporary AIClient instance with the configured settings
        ai_client = AIClient()
        # Override the API key and model with the configured values
        ai_client.api_key = api_key
        ai_client.text_model = model
        ai_client.client = None  # Reset client to reinitialize with new key

        # Reinitialize with the configured API key
        if not ai_client.api_key:
            return JsonResponse({'success': False, 'error': 'OpenAI API key not configured.'}, status=400)

        try:
            ai_client.client = openai.OpenAI(api_key=api_key)
            ai_client.using_v1 = True
        except Exception:
            openai.api_key = api_key
            ai_client.using_v1 = False
...
- Respond in the same language as the user's message"""
                ...
                # Generate streaming response using appropriate API
                logger.info(f"Generating streaming response for message: {message_content[:100]}...")
                if ai_client.using_v1:
                    response = ai_client.client.chat.completions.create(
                    ...
                    # For v0 API, use non-streaming and simulate streaming
                ...
                logger.debug(f"Streaming response completed: {full_response[:100]}...")
